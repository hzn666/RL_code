...
class PPOAgent:
    ...
    def step(self, action: np.ndarray):
        ...
        if not self.is_test:
            self.rewards.append(torch.as_tensor(reward, dtype=torch.float32).to(self.device))
            self.masks.append(torch.as_tensor(1 - done, dtype=torch.float32).to(self.device))
    ...
    def _log(self,
        ...
        print(
            "frame: {} | Score: {} | ActorLoss: {} | CriticLoss: {}".format(frame_idx - 1, score, actor_loss,
                                                                            critic_loss))

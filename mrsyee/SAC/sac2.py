...
class SACAgent:
    ...
    def __init__(
        ...
        self.qf_1_target = CriticQ(obs_dim + action_dim).to(self.device)
        self.qf_1_target.load_state_dict(self.qf_1.state_dict())

        self.qf_2 = CriticQ(obs_dim + action_dim).to(self.device)
        self.qf_2_target = CriticQ(obs_dim + action_dim).to(self.device)
    ...
    def update_model(self):
        ...
        q_1_targ = self.qf_1_target(next_state, next_action)
        q_2_targ = self.qf_2_target(next_state, next_action)
        q_target = torch.min(q_1_targ, q_2_targ)
        q_target_backup = reward + self.gamma * mask * (q_target - alpha + log_prob)
        qf_1_loss = F.mse_loss(q_1_pred, q_target_backup.detach())
        qf_2_loss = F.mse_loss(q_2_pred, q_target_backup.detach())
        ...
        if self.total_step % self.policy_update_freq == 0:
            # actor loss
            q_1_pred = self.qf_1(state, new_action)
            q_2_pred = self.qf_2(state, new_action)
            q_pred = torch.min(q_1_pred, q_2_pred)
    ...
    def _target_soft_update(self):
        ...
        for t_param, l_param in zip(
                self.qf_1_target.parameters(), self.qf_1.parameters()
        ):
            t_param.data.copy_(tau * l_param.data + (1.0 - tau) * t_param.data)

        for t_param, l_param in zip(
                self.qf_2_target.parameters(), self.qf_2.parameters()
        ):
            t_param.data.copy_(tau * l_param.data + (1.0 - tau) * t_param.data)
    ...
    def _log(self,
        ...
        qf_loss = np.mean(qf_losses[-10:])
        alpha_loss = np.mean(alpha_losses[-10:])
        score = np.mean(scores[-10:])
        self.writer.add_scalar('actor_loss', actor_loss, global_step=frame_idx)
        self.writer.add_scalar('qf_loss', qf_loss, global_step=frame_idx)
        self.writer.add_scalar('alpha_loss', alpha_loss, global_step=frame_idx)
        self.writer.add_scalar('score', score, global_step=frame_idx)
        print(
            "frame: {} | Score: {} | ActorLoss: {} | qfLoss: {} | alphaLoss: {}".format(frame_idx, score,
                                                                                        actor_loss,
                                                                                        qf_loss,
                                                                                        alpha_loss))
